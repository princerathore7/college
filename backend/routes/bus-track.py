...
# ======================================================
@bus_bp.route("/api/admin/bus/create", methods=["POST"])
def create_bus():
    data = request.json
...
# ======================================================
# ADMIN: DELETE BUS PILOT ID (FULL BLOCK)
# ======================================================
@bus_bp.route("/api/admin/bus/delete/<busId>", methods=["DELETE"])
...
# BUS DRIVER LOGIN
# ======================================================
@bus_bp.route("/api/bus/login", methods=["POST"])
def bus_login():
    data = request.json
    busId = data.get("busId")
    password = data.get("password")
    ...
    return jsonify({"status": "login_success", "busId": busId})


# ======================================================
# BUS DRIVER: LOCATION UPDATE (ON)
# ======================================================
@bus_bp.route("/api/bus/update-location", methods=["POST"])
def update_location():
    data = request.json
    busId = data.get("busId")

    # check bus active or not
    bus = bus_pilots.find_one({"busId": busId, "active": True})
    if not bus:
...
# ======================================================
# BUS DRIVER: LOCATION OFF (MANUAL STOP)
# ======================================================
@bus_bp.route("/api/bus/stop-location/<busId>", methods=["POST"])
...
# ======================================================
@bus_bp.route("/api/bus/list", methods=["GET"])
def list_buses():
...
# ======================================================
@bus_bp.route("/api/bus/location/<busId>", methods=["GET"])
